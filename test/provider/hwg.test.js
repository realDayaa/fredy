...
import * as provider from '../../lib/provider/hwg.js';
import { launchBrowser, closeBrowser } from '../../lib/services/extractor/puppeteerExtractor.js';

const TEST_TIMEOUT = 120_000;

describe('#hwg testsuite()', () => {
  let browser;

  beforeAll(async () => {
    browser = await launchBrowser(providerConfig.hwg.url);
  }, TEST_TIMEOUT);
  ...
  it(
    ...
    async () => {
      ...
      const runConfig = provider.createConfig(providerConfig.hwg, [], []);
      ...
      expect(notificationObj.serviceName).toBe('hwg');
      notificationObj.payload.forEach((notify) => {
        ...
        expect(notify.link).toContain('https://www.wohnen-halle.de');
