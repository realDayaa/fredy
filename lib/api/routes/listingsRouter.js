...
import { getCachedListingImage } from '../../services/images/listingImageCache.js';
import { reloadListingImageFromSource } from '../../services/images/listingImageReload.js';
...
export default async function listingsPlugin(fastify) {
  ...
  /*
   * Serve the image a user actually sees, decoupled from whatever the source portal currently
   * returns at `image_url`. Some portals (Frohe Zukunft's WordPress theme is the confirmed case)
   * later rotate a different photo, or a generic placeholder, in behind the exact same URL - hence
   * caching the first successful download and serving that copy indefinitely, rather than the
   * frontend hotlinking the origin directly.
   *
   * Listings stored before this cache existed have nothing to serve here, so those fall back to a
   * redirect to the live `image_url` - the same hotlinking behavior the UI always had.
   */
  fastify.get('/:listingId/image', async (request, reply) => {
    const { listingId } = request.params;
    const userId = request.session?.currentUser;
    if (!listingStorage.userCanAccessListing(listingId, userId, isAdminFn(request))) {
      return reply.code(403).send({ message: NO_ACCESS_MESSAGE });
    ...
    const cached = getCachedListingImage(listingId);
    if (cached) {
    ...
    if (listing?.image_url) {
      return reply.redirect(listing.image_url);
    }
    return reply.code(404).send({ message: 'No image available for this listing' });
  });

  /*
   * Force a fresh download of a listing's image, replacing whatever is cached.
   *
   * The one thing the automatic cache cannot fix on its own: a listing whose very first crawl
   * happened to catch a placeholder before a real photo ever existed, or whose stored URL has since
   * gone dead (some portals re-import their whole media library under a new path over time). This
   * is the escape hatch for both cases - deliberately not something the crawl pipeline ever calls
   * itself. `reloadListingImageFromSource` asks the provider for a current URL when it knows how to
   * find one, and falls back to the stored URL otherwise.
   */
  fastify.post('/:listingId/image/refresh', async (request, reply) => {
    const { listingId } = request.params;
    const userId = request.session?.currentUser;
    if (!listingStorage.userCanAccessListing(listingId, userId, isAdminFn(request))) {
      return reply.code(403).send({ message: NO_ACCESS_MESSAGE });
    ...
    const listing = listingStorage.getListingById(listingId, userId, isAdminFn(request));
    if (!listing?.image_url && !listing?.link) {
      return reply.code(404).send({ message: 'This listing has no image URL or link to refresh from' });
    }

    try {
      const refreshed = await reloadListingImageFromSource(listing);
      if (!refreshed) {
        return reply.code(502).send({ message: 'Could not download a fresh image for this listing' });
      }
    } catch (error) {
      logger.error(error);
      return reply.code(500).send({ message: 'Failed to refresh listing image' });
    }
    return reply.send();
  });
