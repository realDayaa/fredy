...
const BASE_URL = 'https://www.hallebwg.de';
...
const config = {
  url: null,
  // Same markup family as WG Eisenbahn: both sites are built on the same TYPO3 property-list
  // extension, so an empty result set means the whole `.row.immobilien` frame is omitted rather
  // than rendered empty - there is nothing to wait for on days without a single vacancy.
