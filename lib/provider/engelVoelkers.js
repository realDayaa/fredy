...
/**
 * Engel & Völkers' search backend answers with a 500 whenever `placeName` is a bare postal code
 * (e.g. "06108") - reproducible on the live site independently of Fredy, presumably because their
 * SSR tries to treat it as a display label rather than geocoding it. The parameter only labels the
 * search for their UI; the actual geo filter comes from `placeIds[]` + `searchRadius`, both of which
 * stay untouched, so it is safe to drop whenever it looks like a postal code instead of a place name.
 *
 * @param {string} url the configured search url
 * @returns {string} the same url with a numeric-only placeName removed
 */
export function stripBrokenPlaceName(url) {
  try {
    const parsed = new URL(url);
    const placeName = parsed.searchParams.get('placeName');
    if (placeName != null && /^\d+$/.test(placeName)) {
      parsed.searchParams.delete('placeName');
    }
    return parsed.href;
  } catch {
    return url;
...
async function getListings(url, browser) {
  const html = await puppeteerExtractor(stripBrokenPlaceName(url), 'body', { browser, name: 'engelVoelkers' });
