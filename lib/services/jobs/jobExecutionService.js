...
export function initJobExecutionService({ providers, intervalMs }) {
  ...
  async function executeJob(job) {
    ...
    try {
      ...
      if (browser) {
        // Must never throw past this point: markFinished() below clears the "running" guard,
        // and skipping it would leave the job stuck as running forever, blocking all future
        // runs until a restart.
        try {
          await puppeteerExtractor.closeBrowser(browser);
        } catch (err) {
          logger.warn('Failed to close browser for job', job.id, err);
        }
      }
      markFinished(job.id);
      try {
