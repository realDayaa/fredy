...
export function parse(crawlContainer, crawlFields, text, url) {
  if (!text) {
    logger.warn('No page content to parse for ', url);
    return null;
  }
  ...
  if ($(crawlContainer).length === 0) {
    logger.warn(`No elements matching crawl container selector found for ${url}. The page structure may have changed or the page didn't load fully.`);
    return null;
  }
