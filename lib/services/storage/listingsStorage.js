...
/**
 * Overwrite the image URL a portal reported for a listing.
 *
 * Every other field the pipeline scrapes is set once at insert and never touched again - see
 * {@link storeListings}. The image URL is the one exception: some portals (Frohe Zukunft's
 * WordPress/MIA-import setup is the confirmed case) periodically re-import their whole media
 * library under a new hashed directory, which leaves the URL captured at crawl time permanently
 * dead even though the same photo still exists, just filed elsewhere. This lets a manual reload
 * record wherever the portal says the photo lives now, without reopening the "never updates a
 * stored field" guarantee everything else in this table relies on.
 *
 * @param {string} id - The listing ID.
 * @param {string} imageUrl
 * @returns {void}
 */
export const updateListingImageUrl = (id, imageUrl) => {
  SqliteConnection.execute(`UPDATE listings SET image_url = @imageUrl WHERE id = @id`, { id, imageUrl });
};
