...
import { detectImageMimeType } from '../services/images/imageFormat.js';
...
export function createMcpServer() {
  ...
  server.tool(
    ...
    async ({ listingId }, extra) => {
      ...
      // The cache holds the photo Fredy first captured for this listing, which some portals later
      // replace at the same URL with a rotated image or a generic placeholder. Preferring it here
      // means vision analysis sees the same photo the UI does, not whatever the origin serves today.
      const cached = getCachedListingImage(listingId);
      if (cached) {
        return {
          content: [{ type: 'image', data: cached.data.toString('base64'), mimeType: cached.mimeType }],
        };
      }
      ...
      const resolvedMime = detectImageMimeType(new Uint8Array(buffer), headerMimeType);
      if (!resolvedMime) {
        return normalizeError(
