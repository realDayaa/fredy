 ...
 * @property {(listing: {link: string|null}) => Promise<string|null>} [rediscoverImage] Re-reads a stored listing's current image URL straight off its live detail page. Only needed by portals whose image URLs can go stale after the listing was first stored (a WordPress media re-import, a CDN path rotation); most providers have stable URLs and omit this. Called on demand by a user-triggered image refresh, never by the crawl pipeline. Must resolve to null rather than reject on any failure.
