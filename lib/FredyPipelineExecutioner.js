...
class FredyPipelineExecutioner {
   ...
   * Runs fetches sequentially with a random delay (2–5 s) between requests to
   * avoid triggering rate limits. Each individual fetch must handle its own errors
  ...
  async _fetchDetails(newListings) {
    ...
    for (let i = 0; i < listingsToEnrich.length; i++) {
      ...
      enriched.push(await this._providerConfig.fetchDetails(listingsToEnrich[i], this._browser));
    }
    return enriched;
  }
