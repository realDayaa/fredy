...
import { cacheListingImageIfMissing } from './services/images/listingImageCache.js';
 ...
 * 11) Cache each listing's image the first time it is seen
 * 12) Calculate distances to the user's addresses
 * 13) Filter out entries similar to already seen ones
 * 14) Filter out entries that do not match the job's spatialFilter
 * 15) Measure each listing against what a square metre costs around it
 * 16) Read each listing for the marks of a rental scam
 * 17) Look up travel times to the user's addresses
 * 18) Apply the job's commute filter to the entries that are too far away
 * 19) Dispatch notifications
 *
...
class FredyPipelineExecutioner {
  ...
  execute() {
    return (
      Promise.resolve(urlModifier(this._providerConfig.url, this._providerConfig.sortByDateParam))
        ...
        .then(this._cacheImages.bind(this))
  ...
  /**
   * Download and cache each new listing's image, right after it was first stored.
   *
   * This is the only point in the pipeline where a listing is guaranteed to be seen for the first
   * time - `_findNew` has already filtered out everything with a known hash - which makes it the
   * right moment to snapshot the photo a portal is showing right now. Some portals later rotate in a
   * different image, or a generic placeholder, at that same URL; caching here means Fredy keeps
   * showing what it originally found instead of silently reflecting whatever the origin serves next.
   *
   * Runs sequentially like {@link _geocode}, and each download already never throws on its own
   * (see {@link module:listingImageCache.cacheListingImageIfMissing}), so a slow or broken image
   * never aborts the rest of the batch.
   *
   * @param {ParsedListing[]} newListings Listings that were just persisted.
   * @returns {Promise<ParsedListing[]>} The same listings, unchanged.
   */
  async _cacheImages(newListings) {
    for (const listing of newListings) {
      await cacheListingImageIfMissing(listing.id, listing.image);
    }
    return newListings;
  }
